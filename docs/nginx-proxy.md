...
NullNode bootstrap and relay can run behind nginx on port 443 with TLS termination.
...
proxy_pass http://127.0.0.1:9001  (ws:// plaintext, localhost only)
    │
    ▼
nullnode-bootstrap (listening on 127.0.0.1:9001)
```
...
# Upstream for the NullNode bootstrap daemon
upstream nullnode_bootstrap {
...
server {
    ...
    # WebSocket endpoint → NullNode daemon
    location /ws {
        limit_req zone=ws burst=20 nodelay;

        proxy_pass http://nullnode_bootstrap;
