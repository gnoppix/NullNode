...
///
/// Reads `~/.nullnode/gnupg/own_cert.asc` (armored public key) and
/// extracts the fingerprint.
pub fn get_own_fingerprint() -> CryptoUtilsResult<String> {
    let path = dirs::home_dir()
        .map(|h| h.join(".nullnode/gnupg/own_cert.asc"))
...
pub fn set_key_trust(fingerprint: &str, trust_level: &str) -> CryptoUtilsResult<()> {
    ...
    let trust_path = dirs::home_dir()
        .map(|h| h.join(".nullnode/gnupg/trust_map.txt"))
        .ok_or_else(|| CryptoUtilsError::Io(
            std::io::Error::new(std::io::ErrorKind::NotFound, "home dir not found")
        ))?;
...
///
/// SECURITY FIX (M1): Uses BLAKE2b-8 (via nullnode-protocol) for consistency
...
pub fn null_id_from_fingerprint(fp: &str) -> String {
    ...
    let hex = nullnode_protocol::pow::blake2b_8_hex(&cleaned);
    let part1 = &hex[..4];
    let part2 = &hex[4..8];
...
mod tests {
    ...
    fn test_secure_delete_nonexistent() {
        ...
        assert!(secure_delete("/tmp/nonexistent_nullnode_test_file").is_ok());
    }

    #[test]
    fn test_secure_delete_existing() {
        let path = "/tmp/nullnode_secure_delete_test";
