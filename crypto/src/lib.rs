...
//-------------------------------------------------------------------------------
// NullNode Crypto — Kyber KEM + Double Ratchet for all user messages.
//-------------------------------------------------------------------------------
...
impl DoubleRatchetSession {
    pub fn new(
        ...
        hk.expand(b"nullnode-double-ratchet-v1-root", &mut root_key)
            .map_err(|_| CryptoError::Ratchet("HKDF root_key".into()))?;
        hk.expand(b"nullnode-double-ratchet-v1-chains", &mut chain_keys)
    ...
    fn derive_message_key(chain_key: &[u8]) -> Result<(Vec<u8>, Vec<u8>), CryptoError> {
        ...
        hk.expand(b"nullnode-double-ratchet-v1-step", &mut output)
    ...
    pub fn encrypt_first(
        ...
        self.state.send_chain_key = new_ck;
    ...
    pub fn decrypt_message(
        ...
        let pt = if body.len() > 2 {
            ...
            if kyber_len > 0 && kyber_len + 2 <= body.len() {
                ...
                self.simple_decrypt(nonce.as_slice(), body)?
            }
        } else {
    ...
    fn simple_decrypt(&mut self, nonce_bytes: &[u8], body: &[u8]) -> Result<Vec<u8>, CryptoError> {
